...
public class RenderGrappleHook extends Render {
	...
	public void doRenderGrappleHook(EntityGrappleHook entityGrappleHook, double par2, double par4, double par6, float par8, float par9)
    {
        ...
        this.bindEntityTexture(entityGrappleHook);
        */
       ...
       EntityPlayer ownerPlayer = entityGrappleHook.getOwner();
        ...
        {
        	/*
            float f9 = ownerPlayer.getSwingProgress(par9);
            float f10 = MathHelper.sin(MathHelper.sqrt_float(f9) * (float)Math.PI);
            Vec3 vec3 = entityGrappleHook.worldObj.getWorldVec3Pool().getVecFromPool(-0.5D, 0.03D, 0.8D);
            vec3.rotateAroundX(-(ownerPlayer.prevRotationPitch + (ownerPlayer.rotationPitch - ownerPlayer.prevRotationPitch) * par9) * (float)Math.PI / 180.0F);
            vec3.rotateAroundY(-(ownerPlayer.prevRotationYaw + (ownerPlayer.rotationYaw - ownerPlayer.prevRotationYaw) * par9) * (float)Math.PI / 180.0F);
            vec3.rotateAroundY(f10 * 0.5F);
            vec3.rotateAroundX(-f10 * 0.7F);
            double d3 = ownerPlayer.prevPosX + (ownerPlayer.posX - ownerPlayer.prevPosX) * (double)par9 + vec3.xCoord;
            double d4 = ownerPlayer.prevPosY + (ownerPlayer.posY - ownerPlayer.prevPosY) * (double)par9 + vec3.yCoord;
            double d5 = ownerPlayer.prevPosZ + (ownerPlayer.posZ - ownerPlayer.prevPosZ) * (double)par9 + vec3.zCoord;
            double d6 = ownerPlayer == Minecraft.getMinecraft().thePlayer ? 0.0D : (double)ownerPlayer.getEyeHeight();

            if (this.renderManager.options.thirdPersonView > 0 || ownerPlayer != Minecraft.getMinecraft().thePlayer)
            {
                float f11 = (ownerPlayer.prevRenderYawOffset + (ownerPlayer.renderYawOffset - ownerPlayer.prevRenderYawOffset) * par9) * (float)Math.PI / 180.0F;
                double d7 = (double)MathHelper.sin(f11);
                double d8 = (double)MathHelper.cos(f11);
                d3 = ownerPlayer.prevPosX + (ownerPlayer.posX - ownerPlayer.prevPosX) * (double)par9 - d8 * 0.35D - d7 * 0.85D;
                d4 = ownerPlayer.prevPosY + d6 + (ownerPlayer.posY - ownerPlayer.prevPosY) * (double)par9 - 0.45D;
                d5 = ownerPlayer.prevPosZ + (ownerPlayer.posZ - ownerPlayer.prevPosZ) * (double)par9 - d7 * 0.35D + d8 * 0.85D;
            }

            double d9 = entityGrappleHook.prevPosX + (entityGrappleHook.posX - entityGrappleHook.prevPosX) * (double)par9;
            double d10 = entityGrappleHook.prevPosY + (entityGrappleHook.posY - entityGrappleHook.prevPosY) * (double)par9 + 0.25D;
            double d11 = entityGrappleHook.prevPosZ + (entityGrappleHook.posZ - entityGrappleHook.prevPosZ) * (double)par9;
            ...
            */
            ...
            for (int i = 0; i <= b2; ++i)
            {
                ...
                System.out.println("Correct Vector: " + (par2 + d12 * (double)f12) + ", " + (par4 + d13 * (double)(f12 * f12 + f12) * 0.5D + 0.25D) + ", " + (par6 + d14 * (double)f12) + ")");
            }
            */
	...
	protected void kuroiYami(EntityGrappleHook entityGrappleHook, double par2, double par4, double par6, float par8, float par9)
    {
        ...
        {
            par4 -= (1.6D - (double)ownerPlayer.height) * 0.5D;
            Tessellator tessellator = Tessellator.instance;
            double d3 = this.func_110828_a((double)entityGrappleHook.prevRotationYaw, (double)entityGrappleHook.rotationYaw, (double)(par9 * 0.5F)) * 0.01745329238474369D;
            double d4 = this.func_110828_a((double)entityGrappleHook.prevRotationPitch, (double)entityGrappleHook.rotationPitch, (double)(par9 * 0.5F)) * 0.01745329238474369D;
			...
			*/
            ...
            double d9 = this.func_110828_a(entityGrappleHook.prevPosX, entityGrappleHook.posX, (double)par9) - d5 * 0.7D - d6 * 0.5D * d8;
            double d10 = this.func_110828_a(entityGrappleHook.prevPosY + (double)entityGrappleHook.getEyeHeight() * 0.7D, entityGrappleHook.posY + (double)entityGrappleHook.getEyeHeight() * 0.7D, (double)par9) - d7 * 0.5D - 0.25D;
            double d11 = this.func_110828_a(entityGrappleHook.prevPosZ, entityGrappleHook.posZ, (double)par9) - d6 * 0.7D + d5 * 0.5D * d8;
            double d12 = this.func_110828_a((double)ownerPlayer.prevRenderYawOffset, (double)ownerPlayer.renderYawOffset, (double)par9) * 0.01745329238474369D + (Math.PI / 2D);
            d5 = Math.cos(d12) * (double)ownerPlayer.width * 0.4D;
            d6 = Math.sin(d12) * (double)ownerPlayer.width * 0.4D;
            double d13 = this.func_110828_a(ownerPlayer.prevPosX, ownerPlayer.posX, (double)par9) + d5;
            double d14 = this.func_110828_a(ownerPlayer.prevPosY, ownerPlayer.posY, (double)par9);
            double d15 = this.func_110828_a(ownerPlayer.prevPosZ, ownerPlayer.posZ, (double)par9) + d6;
            par2 += d5;
            par6 += d6;
